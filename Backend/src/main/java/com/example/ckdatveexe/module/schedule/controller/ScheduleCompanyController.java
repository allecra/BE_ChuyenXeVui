...
public class ScheduleCompanyController {
    ...
    public ResponseEntity<ApiResponse<Page<ScheduleResponse>>> getAllSchedules(
        ...
        try {
            ...
            Page<ScheduleResponse> schedules = scheduleService.getAllSchedulesForCompany(busCompanyId, request);
    ...
    public ResponseEntity<ApiResponse<ScheduleResponse>> getScheduleDetail(
        ...
        try {
            Integer busCompanyId = userDetails.getUser().getBusCompany().getId();
            ScheduleResponse schedule = scheduleService.getScheduleDetailForCompany(scheduleId, busCompanyId);
            ...
            log.error("💥 [COMPANY] 500 INTERNAL_SERVER_ERROR - Failed to get schedule detail: {}", scheduleId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<ScheduleResponse>builder()
                            .success(false)
    ...
    public ResponseEntity<ApiResponse<ScheduleResponse>> updateSchedule(
        ...
        try {
            ...
            log.error("💥 [COMPANY] 500 INTERNAL_SERVER_ERROR - Failed to update schedule: {}", scheduleId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<ScheduleResponse>builder()
                            .success(false)
    ...
    public ResponseEntity<ApiResponse<Page<ScheduleResponse>>> searchSchedules(
        ...
        try {
            ...
            if (status != null) {
                request.setStatus(com.example.ckdatveexe.shared.entity.ScheduleStatus.valueOf(status.toUpperCase()));
            }
            ...
            Page<ScheduleResponse> schedules = scheduleService.searchSchedulesForCompany(busCompanyId, request);

            log.info("✅ [COMPANY] 200 OK - Found {} schedules matching search criteria", schedules.getTotalElements());
            return ResponseEntity.ok(ApiResponse.<Page<ScheduleResponse>>builder()
                    .success(true)
    ...
    public ResponseEntity<ApiResponse<Void>> cancelSchedule(
        ...
        try {
            ...
            log.error("💥 [COMPANY] 500 INTERNAL_SERVER_ERROR - Failed to cancel schedule: {}", scheduleId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<Void>builder()
                            .success(false)
    ...
    public ResponseEntity<ApiResponse<List<ScheduleBusResponse>>> getBusesInSchedule(
        ...
        try {
            Integer busCompanyId = userDetails.getUser().getBusCompany().getId();
            List<ScheduleBusResponse> scheduleBuses = scheduleBusService.getBusesInSchedule(scheduleId, busCompanyId);

            log.info("✅ [COMPANY] 200 OK - Retrieved {} buses for schedule {}", scheduleBuses.size(), scheduleId);
            ...
            log.error("💥 [COMPANY] 500 INTERNAL_SERVER_ERROR - Failed to get buses in schedule: {}", scheduleId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<List<ScheduleBusResponse>>builder()
                            .success(false)
