...
public interface BusReviewRepository extends JpaRepository<BusReview, Integer> {
    ...
    @Query("SELECT br FROM BusReview br WHERE br.bus.busCompany.id = :companyId ORDER BY br.createdAt DESC")
    Page<BusReview> findByBusCompanyIdOrderByCreatedAtDesc(@Param("companyId") Integer companyId, Pageable pageable);
