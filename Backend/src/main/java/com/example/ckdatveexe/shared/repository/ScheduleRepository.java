...
public interface ScheduleRepository extends JpaRepository<Schedule, Integer> {
        ...
        @Query(value = """
                        SELECT
                            ...
                            start_station.province as startProvince,
                            end_station.province as endProvince,
                        ...
                        LEFT JOIN tickets t ON t.schedule_id = s.id AND t.status IN ('CONFIRMED', 'COMPLETED')
                        LEFT JOIN stations start_station ON start_station.id = s.start_station_id
                        LEFT JOIN stations end_station ON end_station.id = s.end_station_id
                        WHERE r.status = 'ACTIVE'
                        GROUP BY r.id, r.route_name, start_station.name, end_station.name,
                                 start_station.province, end_station.province, r.descriptions
                        HAVING COUNT(t.id) > 0
                        ORDER BY COUNT(t.id) DESC
                        LIMIT :limit
                        """, nativeQuery = true)
        List<Object[]> findPopularRoutesByBookingCount(@Param("limit") int limit);
}